document.addEventListener("DOMContentLoaded", function () {
    ...
    input.addEventListener("keydown", function (event) {
      if (event.key === "Enter") {
        ...
        const fraseSeparada = (frase) => frase.split(" ");
        let palabraMasLarga = "";
        palabras = fraseSeparada(frase);
        palabras.forEach(palabra => {
            if(palabraMasLarga.length < palabra.length) palabraMasLarga = palabra;
        });
      ...
      resultado1.textContent = `La palabra mas larga es: ${palabraMasLarga}`;
      bandera = true;
